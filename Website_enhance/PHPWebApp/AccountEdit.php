...
<body>
   ...
   $strAction = $_POST['edit_button'];
  // echo"<h1>EDIT account #$ID </h1>";
  $stmt = "SELECT *
   FROM dbo.mailing_list
   WHERE account_id = '$strAction';";
...
//store the account_id in session for further navigation
$_SESSION['account_id'] = $results['account_id'];

echo"<h1>All Student Accounts Under ". $_SESSION['account_id']. "</h1>";
$stmt = 'SELECT st.account_id, st.student_id, st.student_last_name, st.student_first_name
  FROM dbo.student_registration as st
  WHERE st.account_id = '.$_SESSION['account_id'].';';
  $result = sqlsrv_query($conn, $stmt);
 // print_r($result);

echo"<table>";
echo"<thead>";
echo "<tr><th><h3>Acct</h3></th><th><h3>Student</h3></th><th><h3>LName</h3></th><th><h3>FName</h3></th><th><h3>EXPAND</h3></th></tr>\n";
echo "</thead>";
echo "<tbody>";

while($studentresults = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)){

echo "<tr><td><h2>{$studentresults['account_id']} </h2></td><td><h3>{$studentresults['student_id']}</h3></td>
<td><h3>{$studentresults['student_last_name']} </h3></td><td><h3>{$studentresults['student_first_name']} </h3></td>
<td><form action='StudentExpand.php' method='post'><input name='student_edit_button' type='submit' value={$studentresults['student_id']}></form></td></tr>\n";

}
echo"</tbody>";
echo"</table>";

echo'<form action="StudentAddNew.php" method="post">';
...
$customer1_first_name = $results['customer1_first_name'];
$customer1_last_name = $results['customer1_last_name'];
$customer2_first_name = $results['customer2_first_name'];
$customer2_last_name = $results['customer2_last_name'];
$customer3_first_name = $results['customer3_first_name'];
$customer3_last_name = $results['customer3_last_name'];
$company = $results['company'];
$address1 = $results['address1'];
$address2 = $results['address2'];
$city1 = $results['city1'];
$state1 = $results['state1'];
$city2 = $results['city2'];
$state2 = $results['state2'];
$zipcode1 = $results['zipcode1'];
$zipcode2 = $results['zipcode2'];
$plus4_1 = $results['plus4_1'];
$plus4_2 = $results['plus4_2'];
$country_code1 = $results['country_code1'];
$country_code2 = $results['country_code2'];
$country_name1 = $results['country_name1'];
$country_name2 = $results['country_name2'];
$ship_first_name1 = $results['ship_first_name1'];
$ship_first_name2 = $results['ship_first_name2'];
$ship_last_name1 = $results['ship_last_name1'];
$ship_last_name2 = $results['ship_last_name2'];
$ship_company = $results['ship_company'];
$ship_address = $results['ship_address'];
$ship_city = $results['ship_city'];
$ship_state = $results['ship_state'];
$ship_zipcode = $results['ship_zipcode'];
$ship_plus4 = $results['ship_plus4'];

$ship_country_code = $results['ship_country_code'];
$ship_country_name = $results['ship_country_name'];
...
$customer_flag = $results['customer_flag'];
$source = $results['source'];

$batch_label = $results['batch_label'];
$bulk_label = $results['bulk_label'];
$imed_mail_label = $results['imed_mail_label'];
$imed_ship_label = $results['imed_ship_label'];
$mail_exclude = $results['mail_exclude'];
$registration_expire = $results['registration_expire'];
$counseling = $results['counseling'];
$last_access = $results['last_access'];
$last_change = $results['last_change'];
$usps_address_code = $results['usps_address_code'];
$account_id = $results['account_id'];

 echo"<form action = 'AccountEditConfirm.php' method = 'post'>";
 echo"<table>";
echo"<tr><td>";
 ...
 echo"</td></tr><tr><td>";
  ...
  echo"</table>";
  echo"<br>";
 echo"<input type='submit' name = 'EDIT' id='EDIT' value='UPDATE ACCOUNT'>";
echo"</form>";
